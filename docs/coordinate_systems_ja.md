...
- **例外**：ENVI-met エクスポータは内部を south_up のまま保持し、行を北先頭で書き出す整形をファイル形式境界で行う（`arr[::-1]`）。これは意図的な設計で、`ensure_orientation()` は呼ばない（`tests/test_exporter_envimet.py` の `TestEnvimetSouthUpProcessing` がこれを保証）
