...
The land-cover colour palette and the GeoJSON builders for buildings, canopy
strips, and land-cover grids now live in :mod:`voxcity.geoprocessor.geojson`
(the stable public surface for headless consumers); they are re-imported here
so draw-internal call sites are unchanged.
"""
...
# Re-imports for backward compatibility: these helpers moved to the public
# ``voxcity.geoprocessor.geojson`` module (pure numpy/shapely, no ipyleaflet).
